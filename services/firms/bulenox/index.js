...
class BulenoxService {
    ...
    async processQuery(query) {
        try {
            ...
            if (this.aiMatcher) {
                const aiResult = await this.aiMatcher.findBestFAQ(query, this.faqsCache, this.BULENOX_FIRM_NAME);
                if (aiResult.found) {
            ...
            const logEntry = `${new Date().toISOString()}|${this.BULENOX_FIRM_NAME}|${query}|NO_MATCH\n`;
    ...
    findFAQMatches(query) {
        ...
        if (matches.length > 0) {
            console.log(`[${this.BULENOX_FIRM_NAME}] Query: "${query}"`);
